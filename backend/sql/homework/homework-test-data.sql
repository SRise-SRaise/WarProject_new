...
VALUES (1, '以下哪项最适合作为角色旅程的首层分段？', 'A. 按数据库表名分段,B. 按用户任务阶段分段,C. 按代码文件目录分段', 'B', 2, 5, NULL, '角色旅程应按用户任务阶段拆分。', 2, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), options_text = VALUES(options_text), standard_answer = VALUES(standard_answer), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

INSERT INTO edu_exercise_item (id, question, options_text, standard_answer, question_type, max_score, question_bank_id, analysis, difficulty, created_at, updated_at)
VALUES (2, '验收边界描述中以下哪些属于关键内容？（多选）', 'A. 通过条件,B. 失败条件,C. 界面颜色与字号,D. 开发者个人偏好', 'AB', 3, 5, NULL, '验收边界聚焦通过/失败条件，不包含个人偏好。', 3, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), options_text = VALUES(options_text), standard_answer = VALUES(standard_answer), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

INSERT INTO edu_exercise_item (id, question, options_text, standard_answer, question_type, max_score, question_bank_id, analysis, difficulty, created_at, updated_at)
VALUES (3, '角色旅程图只需要描述主流程，不需要描述异常流。', '', 'F', 4, 3, NULL, '异常流同样重要，应在旅程中明确。', 2, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), standard_answer = VALUES(standard_answer), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

INSERT INTO edu_exercise_item (id, question, options_text, standard_answer, question_type, max_score, question_bank_id, analysis, difficulty, created_at, updated_at)
VALUES (4, '在需求分析中，___是用来描述用户与系统交互过程的工具，而___则定义了系统在何种条件下可以通过验收。', '角色旅程图|验收边界', '角色旅程图|验收边界', 1, 6, NULL, '本题考察角色旅程图与验收边界的概念区分。', 3, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), options_text = VALUES(options_text), standard_answer = VALUES(standard_answer), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

INSERT INTO edu_exercise_item (id, question, options_text, standard_answer, question_type, max_score, question_bank_id, analysis, difficulty, created_at, updated_at)
VALUES (5, '请简述角色旅程图的核心价值，并结合教学平台案例说明其在需求分析中的作用。', '', '', 5, 10, NULL, '主观题，用于评估理解深度。', 4, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

INSERT INTO edu_exercise_item (id, question, options_text, standard_answer, question_type, max_score, question_bank_id, analysis, difficulty, created_at, updated_at)
VALUES (6, 'Vue3的组合式API使用哪个函数创建响应式数据？', 'A. reactive,B. ref,C. computed,D. watch', 'B', 2, 5, NULL, 'ref 常用于基础类型响应式数据。', 2, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), options_text = VALUES(options_text), standard_answer = VALUES(standard_answer), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

INSERT INTO edu_exercise_item (id, question, options_text, standard_answer, question_type, max_score, question_bank_id, analysis, difficulty, created_at, updated_at)
VALUES (7, '以下哪些是Vue3的特性？（多选）', 'A. 组合式API,B. 更好的TypeScript支持,C. 更快的渲染性能,D. 兼容IE8', 'ABC', 3, 5, NULL, 'Vue3 不支持 IE8。', 2, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), options_text = VALUES(options_text), standard_answer = VALUES(standard_answer), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

INSERT INTO edu_exercise_item (id, question, options_text, standard_answer, question_type, max_score, question_bank_id, analysis, difficulty, created_at, updated_at)
VALUES (8, '请简述Vue3相比Vue2的主要改进。', '', '', 5, 20, NULL, '主观题，考察框架演进理解。', 4, NOW(), NOW())
ON DUPLICATE KEY UPDATE question = VALUES(question), question_type = VALUES(question_type), max_score = VALUES(max_score), analysis = VALUES(analysis), difficulty = VALUES(difficulty);

-- ==========================================
-- 5.1 建立作业-题目关系（rel_exercise_item）
-- ==========================================
...
VALUES (1, 1, 1, 5, NOW(), NOW()),
       ...
       (1, 4, 4, 6, NOW(), NOW()),
       (1, 5, 5, 10, NOW(), NOW())
ON DUPLICATE KEY UPDATE item_order = VALUES(item_order), item_score = VALUES(item_score), updated_at = VALUES(updated_at);

-- 作业2关联题目 6~8
INSERT INTO rel_exercise_item (exercise_id, item_id, item_order, item_score, created_at, updated_at)
VALUES (2, 6, 1, 5, NOW(), NOW()),
       (2, 7, 2, 5, NOW(), NOW()),
       (2, 8, 3, 20, NOW(), NOW())
ON DUPLICATE KEY UPDATE item_order = VALUES(item_order), item_score = VALUES(item_score), updated_at = VALUES(updated_at);
...
SELECT rei.exercise_id, rei.item_order, ei.id AS item_id, ei.question, ei.question_type, rei.item_score
FROM rel_exercise_item rei
JOIN edu_exercise_item ei ON rei.item_id = ei.id
ORDER BY rei.exercise_id, rei.item_order;
